...
func TestExecuteToolsRecoversFromPanic(t *testing.T) {
	...
	// The recovered panic must also surface to the caller as a tool-result
	// event, not be swallowed silently.
	var sawError bool
	for _, e := range events {
		if e.Type == EventToolResult && e.Name == "panic_tool" && e.IsError {
			sawError = true
		}
	}
	if !sawError {
		t.Error("expected an error tool_result event for panic_tool")
	}
}
